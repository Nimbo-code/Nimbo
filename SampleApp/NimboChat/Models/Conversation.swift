...
struct Conversation: Identifiable, Codable, Sendable {
    ...
    mutating func updateLastAssistantMessage(
        ...
        if let content = content { messages[index].content = content }
        if let tps = tokensPerSecond { messages[index].tokensPerSecond = tps }
        if let count = tokenCount { messages[index].tokenCount = count }
        if let shifts = windowShifts { messages[index].windowShifts = shifts }
        if let time = prefillTime { messages[index].prefillTime = time }
        if let tokens = prefillTokens { messages[index].prefillTokens = tokens }
        if let history = historyTokens { messages[index].historyTokens = history }
        if let complete = isComplete { messages[index].isComplete = complete }
        if let cancelled = wasCancelled { messages[index].wasCancelled = cancelled }
        if let reason = stopReason { messages[index].stopReason = reason }
